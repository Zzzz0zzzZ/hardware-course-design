...
start2:sw $9, 0($8)	# 以8号寄存器中的内容为基地址，加上偏移量0后的数据存储器地址的存储单元，存9号寄存器内容
lw $14, 0($8)		# 以8号寄存器中的内容为基地址，加上偏移量0后 的数据存储器地址的存储单元的内容，存入14号寄存器
sw $10,4($8)		# 以8号寄存器的内容为基地址，加上偏移量4后的数据存储器地址的存储单元，存10号寄存器内容
lw $15,4($8)		# 以8号寄存器的内容为基地址，加上偏移量4后的数据存储器地址的存储单元的内容，存入15号寄存器
sw $4, -4($8)		# 以8号寄存器的内容为基地址，加上偏移量-4后的数据存储器地址的存储单元，存4号寄存器内容
lw $18, -4($8)		# 以8号寄存器的内容为基地址，加上偏移量-4后的数据存储器地址的存储单元的内容，存入18号寄存器
